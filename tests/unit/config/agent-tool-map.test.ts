...
describe('AGENT_TOOL_MAP', () => {
	...
	it('architect has the Epic-Mode tools but NOT the opaque-dispatch ones', () => {
		// One-flow enforcement: architect's only sanctioned phase-execution
		// flow when Epic is on is declare_scope → epic_decide_phase →
		// epic_plan_waves → Task (per taskId per wave, all in one message)
		// → epic_record_divergence. `lean_turbo_plan_lanes` is retained for
		// non-Epic Lean Turbo use (the planner stays unchanged for legacy
		// callers; Epic Mode routes through the wave planner instead).
		...
		expect(AGENT_TOOL_MAP.architect).toContain('epic_plan_waves');
		...
		expect(AGENT_TOOL_MAP.architect).not.toContain('epic_run_phase' as never);
