...
export const TOOL_METADATA = {
	...
	lean_turbo_run_phase: {
		...
		// Intentionally NOT in the architect's tool map. Same rationale as
		// `epic_run_phase`: it dispatches coders via opencodeClient
		// internally (outside opencode's Task tracking), so the user can't
		// observe the parallel agents. The architect uses planner + Task
		// dispatch instead — `epic_plan_waves` when Epic Mode is on,
		// `lean_turbo_plan_lanes` otherwise. The function
