...
- Epic Mode now ships Capability D — **outcome-based self-calibration**.
  ...
  subsequent `epic_run_phase`, the calibration engine consumes any new
...
- New module surface in `src/turbo/epic/`:
  ...
  - `calibration-engine.ts` — pure `applyCalibration(state, newRecords,
    ...
    `epic_run_phase` tool consults at decision time.
...
- Existing `epic_run_phase` tool now (a) consumes any new divergence
...
- 50 new tests covering:
  ...
  - `epic_run_phase` (4 new tests) — calibration → activation wiring,
